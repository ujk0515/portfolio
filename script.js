...
const VISITOR_COUNTER_FUNCTION_URL = 'https://ceilxgtgkfvvvfcgkmlx.supabase.co/functions/v1/visitor-counter';
...
async function initVisitorCounter() {
    try {
        // Edge Function 호출 (IP 자동 감지 및 카운팅)
        const response = await fetch(VISITOR_COUNTER_FUNCTION_URL, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            displayVisitorCount(data.totalCount);
            console.log('Visitor tracking:', {
                ip: data.clientIp,
                isNew: data.isNewVisitor,
